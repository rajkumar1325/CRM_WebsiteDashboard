...
const IconBtn = ({ onClick, children, badge, active, title, isDark, btnHover }) => (
  <button
    onClick={onClick}
    title={title}
    className={`
    ...
    <>
      ...
      `}>
        ...
        <div className="flex items-center gap-1.5 shrink-0 ml-auto">
          ...
          {/* Settings */}
          <IconBtn onClick={() => navigate("/dashboard/settings") } title="Settings" 
            isDark={isDark}
            btnHover={btnHover}>
          ...
          <div ref={profileRef} className="relative">
            ...
            {isProfileOpen && (
              <div className={`absolute top-12 right-0 w-52 rounded-2xl border shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 ${dropBg}`}>
                ...
                {[
                  { label: "My Profile",         icon: "👤",    onClick: () => { navigate("/dashboard/settings"); setIsProfileOpen(false); }   },
                  { label: "Account Settings",   icon: "⚙️",    onClick: () => { navigate("/dashboard/settings"); setIsProfileOpen(false); }   },
                  { label: "Billing",            icon: "💳",    onClick: () => { navigate("/dashboard/billing"); setIsProfileOpen(false); }   },
                  { label: "Keyboard Shortcuts", icon: "⌨️",    onClick: () => { navigate("/dashboard/shortcuts"); setIsProfileOpen(false); }   },
                ].map((item) => (
                  <button key={item.label} onClick={item.onClick} className={`flex items-center gap-2.5 w-full px-4 py-2.5 text-left text-sm transition-colors duration-150 ${dropItem}`}>
