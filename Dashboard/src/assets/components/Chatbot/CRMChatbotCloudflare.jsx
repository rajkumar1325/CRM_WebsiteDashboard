...
// ─── CONFIG ───────────────────────────────────────────────────────────────────
const CloudF_Account_id = import.meta.env.VITE_CF_ACCOUNT_ID;
const CloudF_Api_Token  = import.meta.env.VITE_CF_API_TOKEN;
...
function buildCRMContext({ mockData = [], analyticsData = {}, calendarData = [], dealsData = [], tasksData = [], customersData = [], AGENTS = [] } = {}) {
  const leads = mockData.map(l => ({
    ...
    lastContact:    l.callHistory?.length ? l.callHistory[l.callHistory.length - 1].time?.slice(0, 10) : null,
    lastFeedback:   l.callHistory?.length ? l.callHistory[l.callHistory.length - 1].feedback : "No contact yet",
  ...
  const agents    = AGENTS.map(a => ({ name: a.name, role: a.role, leadsAssigned: leads.filter(l => l.assignedTo === a.name).length }));
  ...
  return {
    ...
    reportSummary: { conversionRate: "24%", avgDealValue: "₹1,285", topSource: "Website (12 leads)" },
  };
}
...
- Give specific names/numbers from the data
...
function Bubble({ msg, darkMode }) {
  ...
  return (
    <div className={`chat-row ${isUser ? "chat-row-user" : "chat-row-bot"}`}>
      ...
      <div className={`chat-bubble ${isUser ? "bubble-user" : ""} ${!isUser && !isError ? (darkMode ? "bubble-bot-dark" : "bubble-bot-light") : ""} ${isError ? "bubble-error" : ""}`}>
        <p className="bubble-text">{msg.content}</p>
        <p className={`bubble-time ${isUser ? "time-user" : (darkMode ? "time-bot-dark" : "time-bot-light")}`}>{msg.time}</p>
      </div>
    </div>
  );
}
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function CRMChatbotCloudflare({ darkMode = true, onClose, crmData = {} }) {
  ...
  async function send(text) {
    ...
    try {
      const res = await fetch(CloudF_Api_Url, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${CloudF_Api_Token}`,
        },
        body: JSON.stringify({
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: "", time: fmt(new Date()) }]);
      setIsLoading(false);
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            setMessages((prev) => {
              const updated = [...prev];
              updated[updated.length - 1] = { role: "assistant", content: accumulated, time: fmt(new Date()) };
              return updated;
            });
  ...
  return (
    <>
      <style>{`
        .chat-glass-dark  { background: rgba(13,13,24,0.88); border: 1px solid rgba(255,255,255,0.09); backdrop-filter: blur(40px) saturate(180%); box-shadow: 0 32px 80px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.08); }
        .chat-glass-light { background: rgba(255,255,255,0.82); border: 1px solid rgba(220,220,240,0.9); backdrop-filter: blur(40px) saturate(180%); box-shadow: 0 32px 80px rgba(80,80,140,0.15), inset 0 1px 0 rgba(255,255,255,1); }
        ...
        .chat-hbtn { width:26px; height:26px; border-radius:8px; border:none; display:flex; align-items:center; justify-content:center; font-size:12px; cursor:pointer; transition:background 0.2s; }
        ...
        .chat-messages { flex:1; overflow-y:auto; padding:12px 14px; display:flex; flex-direction:column; scrollbar-width:thin; scrollbar-color:rgba(192,132,252,0.25) transparent; }
        ...
        .bot-avatar-sm { width:24px; height:24px; border-radius:7px; flex-shrink:0; background:linear-gradient(135deg,#818cf8,#c084fc); display:flex; align-items:center; justify-content:center; font-size:11px; color:white; font-weight:700; }
        .user-avatar-sm { width:24px; height:24px; border-radius:7px; flex-shrink:0; background:#3b82f6; color:white; display:flex; align-items:center; justify-content:center; font-size:7.5px; font-weight:800; }
        ...
        @keyframes chatSlideUp { from{opacity:0;transform:translateY(18px) scale(0.97)} to{opacity:1;transform:translateY(0) scale(1)} }
        .chat-entrance { animation:chatSlideUp 0.32s cubic-bezier(0.34,1.4,0.64,1) both; }
      `}</style>

      <div className={`chat-entrance flex flex-col ${darkMode ? "chat-glass-dark" : "chat-glass-light"} fixed bottom-6 right-6 z-[200] w-[370px] h-[580px] rounded-2xl overflow-hidden max-sm:inset-0 max-sm:w-full max-sm:h-full max-sm:rounded-none max-sm:bottom-0 max-sm:right-0`}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:4, marginTop:1 }}>
                ...
                <span className={darkMode ? "chat-status-dark" : "chat-status-light"}>Live data · CuRIEM AI</span>
              </div>
            </div>
          </div>
        ...
        <div className="chat-messages">
          {messages.map((msg, i) => <Bubble key={i} msg={msg} darkMode={darkMode} />)}
