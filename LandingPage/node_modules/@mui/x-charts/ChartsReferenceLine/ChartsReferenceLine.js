...
function ChartsReferenceLine(props) {
  ...
  if (x !== undefined && y !== undefined) {
    throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X Charts: The ChartsReferenceLine cannot have both `x` and `y` props set. ' + 'A reference line can only be vertical (x) or horizontal (y), not both. ' + 'Remove either the `x` or `y` prop.' : (0, _formatErrorMessage2.default)(4));
  }
  if (x === undefined && y === undefined) {
    throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X Charts: The ChartsReferenceLine requires either an `x` or `y` prop. ' + 'Without a value, the reference line cannot be positioned on the chart. ' + 'Provide either an `x` value for a vertical line or a `y` value for a horizontal line.' : (0, _formatErrorMessage2.default)(5));
  }
  if (x !== undefined) {
